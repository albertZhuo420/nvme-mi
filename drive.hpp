...
class Drive {
  public:
	 ...
	 */
	Drive(const std::string &driveName, mctpw::eid_t eid, sdbusplus::asio::object_server &objServer, std::shared_ptr<mctpw::MCTPWrapper> wrapper);
	/**
  ...
  private:
	std::tuple<int, std::string> collectDriveLog(boost::asio::yield_context yield);
