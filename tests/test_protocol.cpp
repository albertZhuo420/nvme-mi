...
TEST(NVMeMsg, Create)
{
	...
	EXPECT_THROW(prot::NVMeMessage<uint8_t *> msg(nullptr, 0), std::invalid_argument);
	...
	std::vector<uint8_t> data4(ConstNVMeMsg::minSize + sizeof(ConstNVMeMsg::CRC32C), 0x00);
	...
	EXPECT_NO_THROW(prot::NVMeMessage msg(data4));
	{
		...
		prot::NVMeMessage msg2(data4, prot::NVMeMessageTye::miCommand, prot::CommandSlot::slot0, true);
...
{
	namespace prot					= nvmemi::protocol;
	std::array<uint8_t, 8> expected = {0x84, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	std::array<uint8_t, 8> testInput {};
	prot::NVMeMessage	   msg(testInput, prot::NVMeMessageTye::miCommand, prot::CommandSlot::slot0, true);
...
TEST(NVMeRsp, Create)
{
	...
	EXPECT_THROW(prot::NVMeResponse msg(static_cast<const uint8_t *>(nullPtr), 0), std::invalid_argument);
	std::array<uint8_t, 3> data {};
	EXPECT_THROW(prot::NVMeResponse msg(data), std::length_error);
	EXPECT_THROW(prot::NVMeResponse msg(static_cast<const uint8_t *>(data.data()), data.size()), std::length_error);
	...
	std::array<uint8_t, ConstResp::minSize + sizeof(ConstResp::CRC32C)> data5 = {0x00, 0x00, 0x00, 0x00, 0x00, 0x35, 0x76, 0x72, 0x45};
	EXPECT_NO_THROW(prot::NVMeResponse msg(data5));
	data5[5] = 0x00;
	EXPECT_ANY_THROW(prot::NVMeResponse msg(data5));
	{
		std::array<uint8_t, 12> data5 = {0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x32, 0x18, 0x6d, 0x51};
		EXPECT_NO_THROW(prot::NVMeResponse msg(data5));
...
TEST(NVMeRsp, GetSet)
{
	namespace prot = nvmemi::protocol;
	{
		std::array<uint8_t, 12> data = {0x084, 0x08, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x51, 0x78, 0x7e, 0x68};
...
TEST(ManagementIntgerfaceMessage, Create)
{
	...
	EXPECT_THROW(prot::ManagementInterfaceMessage<const uint8_t *> msg(nullptr, 0), std::invalid_argument);
	EXPECT_THROW(prot::ManagementInterfaceMessage<uint8_t *> msg(nullptr, 0), std::invalid_argument);
	{
		std::array<uint8_t, 19> data {};
		EXPECT_THROW(prot::ManagementInterfaceMessage msg(data), std::length_error);
		EXPECT_THROW(prot::ManagementInterfaceMessage msg(static_cast<const uint8_t *>(data.data()), data.size()), std::length_error);
...
TEST(ManagementIntgerfaceMessage, GetSet)
{
	namespace prot = nvmemi::protocol;
	{
		const std::vector<uint8_t>		 expected = {0x84, 0x08, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,
													 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
		prot::ManagementInterfaceMessage msg(expected);
		EXPECT_EQ(msg->opCode, prot::MiOpCode::controllerHealthStatusPoll);
		EXPECT_EQ(msg.getMiOpCode(), prot::MiOpCode::controllerHealthStatusPoll);
	}  // namespace nvmemi::protocol;
	std::vector<uint8_t>			 expected = {0x84, 0x08, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,
												 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x82, 0xa8, 0xe5, 0x65};
	std::vector<uint8_t>			 testInput(20);
	prot::ManagementInterfaceMessage msg(testInput, prot::MiOpCode::controllerHealthStatusPoll);
	EXPECT_EQ(expected, testInput);
	EXPECT_EQ(msg.getNvmeMiMsgType(), prot::NVMeMessageTye::miCommand);
...
TEST(ManagementIntgerfaceResponse, GetSet)
{
	...
	uint8_t *nullPtr	  = nullptr;
	EXPECT_THROW(ConstMIResponse msg(nullPtr, ConstMIResponse::minSize + sizeof(ConstMIResponse::CRC32C)), std::invalid_argument);
	EXPECT_THROW(MIResponse msg(nullPtr, ConstMIResponse::minSize + sizeof(ConstMIResponse::CRC32C)), std::invalid_argument);
	{
		std::vector<uint8_t> data = {0x00, 0x00, 0x00, 0x00, 0x00, 0x35, 0x76, 0x72, 0x45};
	...
	{
		std::array<uint8_t, 14> data1 = {0x84, 0x88, 0x0, 0x0, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0xcd, 0x3b, 0xb3, 0xca};
		EXPECT_NO_THROW(prot::ManagementInterfaceResponse response(data1));

		std::array<uint8_t, ConstMIResponse::minSize + sizeof(ConstMIResponse::CRC32C)> data2 =
			{0x84, 0x88, 0x0, 0x0, 0x2, 0x3, 0x4, 0x5, 0x16, 0xc3, 0x45, 0xc};
...
TEST(AdminCommand, Create)
{
	...
	std::vector<uint8_t> test(AdminCmd::minSize + sizeof(AdminCmd::CRC32C), 0x00);
...
{
	std::vector<uint8_t> testData = {0x84, 0x88, 0x0, 0x0, 0x0, 0x0,  0x0,	0x0,  0x0,	0x0,  0x0,	0x0,  0x0,
									 0x0,  0x0,	 0x0, 0x0, 0x0, 0x18, 0x80, 0x12, 0x34, 0x1d, 0x2a, 0x42, 0x49};
	namespace prot				  = nvmemi::protocol;
