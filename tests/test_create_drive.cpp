...
static auto readPropertyValue(sdbusplus::bus::bus &bus,
							  const std::string	  &service,
							  const std::string	  &path,
							  const std::string	  &interface,
							  const std::string	  &property)
{
	std::cout << (std::string("Reading ") + service + " " + path + " " + interface + " " + property + "\n").c_str();
	auto msg = bus.new_method_call(service.c_str(), path.c_str(), "org.freedesktop.DBus.Properties", "Get");
...
void childTask()
{
	...
	auto								  drive = std::make_shared<nvmemi::Drive>("NVMeDrive1", 1, *objectServer, mctpWrapper);
