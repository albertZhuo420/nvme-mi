...
void parentTask()
{
	...
	boost::asio::posix::stream_descriptor stream {*gAppData->ioContext, gAppData->pToC[readIdx]};
	auto								  handler = [&readData](const boost::system::error_code &ec, std::size_t n) {
		 if (ec) {
		 ...
		 boost::asio::spawn([&](boost::asio::yield_context yield) {
			 try {
				 ...
				 status = gAppData->dbusConnection->yield_method_call<decltype(status)>(yield,
																						collectLogError,
																						"xyz.openbmc_project.nvmemi_test",
																						"/xyz/openbmc_project/CollectLogDrive",
																						"xyz.openbmc_project.drive_log",
																						"CollectLog");
				 if (collectLogError) {
					 std::cout << "Error calling collect log. " << collectLogError.message() << '\n';
					 throw collectLogError;
				 }
				 else {
					 std::cout << "Status " << std::get<0>(status) << " Message " << std::get<1>(status) << '\n';
					 std::ifstream		  ifs(std::get<1>(status));
					 nlohmann::json		  nvmeDumpFile = nlohmann::json::parse(ifs);
					 auto				  activeNamespacesActual = nvmeDumpFile["Identify"]["ActiveNamespaces"].get<std::vector<uint8_t>>();
					 std::vector<uint8_t> activeNamespacesExpected = {1};
					 EXPECT_TRUE(activeNamespacesActual == activeNamespacesExpected);
				 }
			 }
	...
	boost::asio::async_read(stream, boost::asio::buffer(&readData, sizeof(readData)), handler);
...
void childTask()
{
	...
	auto objectServer = std::make_shared<sdbusplus::asio::object_server>(gAppData->dbusConnection);
	...
	auto					 mctpWrapper = std::make_shared<mctpw::MCTPWrapper>(gAppData->dbusConnection, config);

	static constexpr auto				  dummyEid = 8;
	auto								  drive	   = std::make_shared<nvmemi::Drive>("CollectLogDrive", dummyEid, *objectServer, mctpWrapper);
	boost::asio::posix::stream_descriptor stream {*gAppData->ioContext, gAppData->cToP[readIdx]};
	uint8_t								  writeData = 'A';

	write(gAppData->pToC[writeIdx], &writeData, sizeof(writeData));
	std::cout << "Waiting for parent to finish unit tests" << '\n';
	auto handler = [&writeData](const boost::system::error_code &ec, std::size_t n) {
		if (ec) {
			std::cout << "Read error. " << ec.message() << '\n';
		}
	...
	boost::asio::async_read(stream, boost::asio::buffer(&writeData, sizeof(writeData)), handler);
...
int main(int argc, char **argv)
{
	...
	gAppData->dbusConnection = std::make_shared<sdbusplus::asio::connection>(*gAppData->ioContext);
