...
void monitorSignal()
{
	...
	signalMatches.emplace_back(*(gAppData->dbusConnection),
							   "type='signal', "
							   ...
							   [](sdbusplus::message::message &message) {
								   SignalData data;
								   try {
									   message.read(data.driveName, data.interface, data.alarm, data.asserted, data.value);
								   }
								   ...
								   std::cout << "Threshold asserted... Alarm " << data.alarm << " for value " << data.value << "\n";

								   EXPECT_EQ(data.value, 120);
								   EXPECT_EQ(data.asserted, true);

								   if (data.interface == "xyz.openbmc_project.Sensor.Threshold.Warning") {
									   EXPECT_EQ(data.alarm, "WarningAlarmHigh");
									   gAppData->signalCaught++;
								   }
								   else if (data.interface == "xyz.openbmc_project.Sensor.Threshold.Critical") {
