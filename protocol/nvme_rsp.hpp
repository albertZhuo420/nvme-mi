...
class NVMeResponse<const uint8_t *> : public virtual NVMeMessage<const uint8_t *> {
  public:
	static constexpr size_t minSize = NVMeMessage<const uint8_t *>::minSize + sizeof(StatusCode);

	NVMeResponse(const uint8_t *data, size_t len) :
	  NVMeMessage<const uint8_t *>(data, len), base(data), status(reinterpret_cast<const uint8_t *>(data + NVMeMessage<const uint8_t *>::minSize))
	{
	...
	{ }
	constexpr uint8_t getStatus() const noexcept { return *status; }
...
template <>
class NVMeResponse<uint8_t *> : public virtual NVMeResponse<const uint8_t *>, public NVMeMessage<uint8_t *> {
  public:
	NVMeResponse(uint8_t *data, size_t len) :
	  NVMeMessage<const uint8_t *>(data, len), NVMeResponse<const uint8_t *>(data, len), NVMeMessage<uint8_t *>(data, len),
	  status(reinterpret_cast<uint8_t *>(data + NVMeMessage<const uint8_t *>::minSize))
	{ }
	template <typename T>
	NVMeResponse(T &&arr) : NVMeResponse(arr.data(), arr.size())
	{ }
	NVMeResponse(uint8_t *data, size_t len, uint8_t errStatus) : NVMeResponse<uint8_t *>(data, len) { *status = errStatus; }
	template <typename T>
	NVMeResponse(T &&arr, uint8_t errStatus) : NVMeResponse(arr.data(), arr.size(), errStatus)
	{ }
